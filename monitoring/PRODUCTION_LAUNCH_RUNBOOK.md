...
| `MinIODown` | SUPERSEDED by `StorageDown` (rules-database.yml): storage is now RustFS, which has no `/minio/v2/metrics/*` endpoints — liveness comes from the DB-VM Alloy blackbox probe (`probe_success{job="storage"}`) | Already enabled; nothing to add |
