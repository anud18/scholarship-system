 ...
 *
 * URL defaults match the localhost dev stack (docker-compose.dev.yml). The
 ...
 *
 * E2E_DATABASE_URL (consumed by helpers/db.ts) has NO default — every lane,
 * including local runs against the dev stack, must export it explicitly with
 * the postgres credentials of the target stack.
 */
