...
test.describe("申請端點的角色權限邊界 | Role-permission boundaries on applications endpoints", { tag: [MODE.api, ROLE.student, ROLE.professor, FEATURE.permissions] }, () => {
  ...
  test("wrong-role callers are 403 on student/professor endpoints", async ({ browser }) => {
    ...
    //    (the route depends on require_professor, which rejects any other role).
    //
    // Body must satisfy the review schema (application_id + items[]) so Pydantic
