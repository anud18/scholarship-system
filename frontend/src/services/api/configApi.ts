...
export const configApi = {
   ...
   */
  getScholarshipTypeConfigs: async (): Promise<
    ApiResponse<ScholarshipTypeConfig[]>
  > => {
  ...
  getMatrixQuotaStatus: async (
    ...
    return apiCall(
      `/api/v1/scholarship-configurations/test/matrix-quota-status/${semester}`
    );
  },
   ...
   */
  getRegionalQuotaStatus: async (
    semester: string = "2025-1"
  ): Promise<ApiResponse<any>> => {
    return apiCall(
      `/api/v1/scholarship-configurations/test/regional-quota-status/${semester}`
    );
  },
