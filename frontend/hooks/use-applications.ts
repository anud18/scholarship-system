...
export function useApplications() {
  ...
  const createApplication = useCallback(
    async (applicationData: ApplicationCreate, isDraft: boolean = false) => {
      try {
        setError(null);

        const response = await apiClient.applications.createApplication(
          applicationData,
          isDraft
        );
  ...
  const withdrawApplication = useCallback(async (applicationId: number) => {
    try {
      ...
      const response =
        await apiClient.applications.withdrawApplication(applicationId);
  ...
  const updateApplication = useCallback(
    async (
      ...
      try {
        ...
        const response = await apiClient.applications.updateApplication(
          applicationId,
          applicationData
        );
  ...
  const uploadDocument = useCallback(
    async (applicationId: number, file: File, fileType: string = "other") => {
      try {
        ...
        const response = await apiClient.applications.uploadDocument(
          applicationId,
          file,
          fileType
        );
  ...
  const saveApplicationDraft = useCallback(
    async (applicationData: ApplicationCreate) => {
      try {
        setError(null);

        const response =
          await apiClient.applications.saveApplicationDraft(applicationData);
  ...
  const deleteApplication = useCallback(async (applicationId: number) => {
    try {
      ...
      const response =
        await apiClient.applications.deleteApplication(applicationId);
