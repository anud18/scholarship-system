...
function SortableItem({ application, index, totalQuota, locale, isFinalized, onReviewApplication, reviewScores, handleScoreUpdate, calculateTotalScore }: {
            ...
            <DialogContent className="max-w-2xl">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium">學術成績 (30%)</label>
                    <Input
                      type="number"
                      min="0"
                      max="100"
                      value={reviewScores[application.id]?.academic || ""}
                      onChange={(e) => handleScoreUpdate(application.id, 'academic', Number(e.target.value))}
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      min="0"
                      max="100"
                      value={reviewScores[application.id]?.professor_review || ""}
                      onChange={(e) => handleScoreUpdate(application.id, 'professor_review', Number(e.target.value))}
                      disabled={isFinalized}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      min="0"
                      max="100"
                      value={reviewScores[application.id]?.college_criteria || ""}
                      onChange={(e) => handleScoreUpdate(application.id, 'college_criteria', Number(e.target.value))}
                      disabled={isFinalized}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      min="0"
                      max="100"
                      value={reviewScores[application.id]?.special_circumstances || ""}
                      onChange={(e) => handleScoreUpdate(application.id, 'special_circumstances', Number(e.target.value))}
                      disabled={isFinalized}
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) => handleScoreUpdate(application.id, 'comments', e.target.value)}
                    disabled={isFinalized}
                    rows={3}
                  />
                </div>
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => handleScoreUpdate(application.id, 'recommendation', value)}
                    disabled={isFinalized}
                  >
                    <SelectTrigger>
...
export function CollegeRankingTable({
  ...
  useEffect(() => {
    ...
    const sortedApps = [...applications].sort((a, b) => a.rank_position - b.rank_position)
  ...
  const allocatedCount = localApplications.filter(app => app.is_allocated).length
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-orange-600">{remainingQuota}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              {!isFinalized && (
                <>
                  <Button variant="outline" size="sm" onClick={onExecuteDistribution}>
                    <Send className="h-4 w-4 mr-2" />
                    {locale === "zh" ? "執行分配" : "Execute Distribution"}
                  </Button>
                  <Button variant="default" size="sm" onClick={onFinalizeRanking}>
                    <Save className="h-4 w-4 mr-2" />
        ...
        <CardContent>
          {isFinalized && (
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-800 flex items-center gap-2">
                ...
                {locale === "zh" ? "此排名已確認，無法再修改" : "This ranking has been finalized and cannot be modified"}
              </p>
            </div>
          )}
          ...
          >
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-12">{locale === "zh" ? "排名" : "Rank"}</TableHead>
                  <TableHead>{locale === "zh" ? "學生" : "Student"}</TableHead>
                  <TableHead className="text-center">{locale === "zh" ? "分數" : "Score"}</TableHead>
                  <TableHead className="text-center">{locale === "zh" ? "狀態" : "Status"}</TableHead>
                  <TableHead className="text-center">{locale === "zh" ? "操作" : "Actions"}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                <SortableContext 
