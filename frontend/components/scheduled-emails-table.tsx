...
export function ScheduledEmailsTable({ className, currentUserRole }: ScheduledEmailsTableProps) {
                    ...
                    <TableRow key={email.id}>
                      <TableCell>
                        <div className="flex flex-col">
                          <span>{new Date(email.scheduled_for).toLocaleString('zh-TW')}</span>
                          {email.is_due && (
                      ...
                      <TableCell>
                        {email.requires_approval ? (
                          email.approved_by_user_id ? (
                            <div className="flex flex-col">
                              <Badge variant="default" className="mb-1">已審核</Badge>
                              {email.approved_at && (
                                <span className="text-xs text-gray-500">
                                  {new Date(email.approved_at).toLocaleDateString('zh-TW')}
                                </span>
                              )}
                            </div>
                          ) : (
                            ...
                            {selectedEmail && (
                              <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-6">
                                  ...
                                  <div className="space-y-2">
                                    <Label className="text-sm font-medium text-gray-700">排程時間</Label>
                                    <p className="text-sm text-gray-900">{new Date(selectedEmail.scheduled_for).toLocaleString('zh-TW')}</p>
                                  </div>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <div>
                                      <Badge variant={getPriorityVariant(selectedEmail.priority)}>
                                        {getPriorityLabel(selectedEmail.priority)}
                                      </Badge>
                                    </div>
                                  </div>
                                  <div className="space-y-2">
                                    <Label className="text-sm font-medium text-gray-700">需要審核</Label>
                                    <div>
                                      <Badge variant={selectedEmail.requires_approval ? 'destructive' : 'secondary'}>
                                        {selectedEmail.requires_approval ? '需要審核' : '不需要審核'}
                                      </Badge>
                                    </div>
                                  </div>
                                  {selectedEmail.approved_by_username && (
                                    <div className="space-y-2">
                                      <Label className="text-sm font-medium text-gray-700">審核者</Label>
                                      <p className="text-sm text-gray-900">{selectedEmail.approved_by_username}</p>
                                    </div>
                                  )}
                                  {selectedEmail.approval_notes && (
                                    <div className="col-span-2 space-y-2">
                                      <Label className="text-sm font-medium text-gray-700">審核備註</Label>
                                      <p className="text-sm text-blue-800 bg-blue-50 p-3 rounded-md border border-blue-200">{selectedEmail.approval_notes}</p>
                                    </div>
                                  )}
                                </div>
                      ...
                      <TableCell>
                        <div className="flex gap-2">
                          {email.status === 'PENDING' && email.requires_approval && !email.approved_by_user_id && currentUserRole === 'super_admin' && (
                            <Button
                              size="sm"
                              variant="outline"
