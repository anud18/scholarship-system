    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const transformedApplications = (response.data.items || []).map((item: any) => ({
          ...
          student_name: item.application?.student_info?.display_name || '未提供姓名',
          student_no: item.application?.student_info?.student_id_masked || 'N/A',
          scholarship_type: item.application?.scholarship_type || '',
          sub_type: item.application?.sub_type || '',
