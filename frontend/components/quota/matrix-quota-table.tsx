...
export function MatrixQuotaTable({
  ...
  const colleges =
    localData && Object.keys(localData.phd_quotas).length > 0
      ? Array.from(
          new Set(
            Object.values(localData.phd_quotas).flatMap(subtypeData =>
              Object.keys(subtypeData)
            )
          )
        ).sort()
      : academyCodes;
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const academicYear = currentPeriod
      ...
      if (response.success && response.data) {
        ...
        updatedData.phd_quotas[editingCell.subType][
          editingCell.college
        ].total_quota = newValue;
        updatedData.phd_quotas[editingCell.subType][
          editingCell.college
        ].available =
          newValue -
          updatedData.phd_quotas[editingCell.subType][editingCell.college].used;
  ...
  const calculateSubTypeTotal = useCallback(
    (subType: string): QuotaCell => {
      if (!localData)
        return { total_quota: 0, used: 0, available: 0, applications: 0 };
  ...
  const calculateCollegeTotal = useCallback(
            ...
            return (
              <TableRow key={subType}>
                ...
                {colleges.map(college => {
                  ...
                  const isEditing =
                    editingCell?.subType === subType &&
                    editingCell?.college === college;
                  const isSaving = savingCell === cellKey;

                  return (
                    <TableCell
                      key={college}
                      className={cn("text-center p-1", getCellColor(cell))}
                    >
                      {isEditing ? (
                        ...
                        <div
                          ...
                          onClick={() =>
                            handleEditStart(subType, college, cell.total_quota)
                          }
                        >
            ...
            <TableCell className="text-center bg-gray-100">
              <div className="flex flex-col items-center">
                <span className="text-lg">
                  {localData.grand_total.total_quota}
                </span>
                <span className="text-xs">
                  {localData.grand_total.total_used}/
                  {localData.grand_total.total_available}
                </span>
              </div>
            </TableCell>
          </TableRow>
