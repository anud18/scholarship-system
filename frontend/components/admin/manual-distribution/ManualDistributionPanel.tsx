...
export function ManualDistributionPanel({
  ...
  const handleCheckbox = (rankingItemId: number, subTypeCode: string) => {
    setLocalAllocations(prev => {
      ...
      next.set(
        rankingItemId,
        next.get(rankingItemId) === subTypeCode ? null : subTypeCode
      );
      return next;
    });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const allocations = Array.from(localAllocations.entries()).map(
        ([ranking_item_id, sub_type_code]: [number, string | null]) => ({
          ranking_item_id,
          sub_type_code,
        })
      );
          ...
          {isLoading ? (
            ...
            <div className="overflow-x-auto">
              <table
                ...
                style={{ minWidth: `${900 + subTypeKeys.length * 90}px` }}
              >
                  ...
                  {subTypeKeys.length > 0 && (
                    <tr className="bg-blue-50/50 text-[11px] text-center">
                      {subTypeKeys.map(key => (
                        <th
                          key={key}
                          className="px-2 py-1.5 border border-slate-200 text-slate-500 whitespace-nowrap"
                        >
                      ...
                      ({
                        ...
                        <>
                          ...
                          {collegeStudents.map(student => {
                            const allocated = localAllocations.get(
                              ...
                              >
                                ...
                                {subTypeKeys.map(key => {
                                  ...
                                  const atCapacity =
                                    ...
                                    localAllocCounts[key] >=
                                      quotaStatus[key].total &&
                                    !isChecked;
                                  return (
                                    <td
                                      key={key}
