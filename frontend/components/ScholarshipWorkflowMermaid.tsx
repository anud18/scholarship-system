...
import { Download, Info, Calendar, Users, Clock, RefreshCw, ChevronRight } from "lucide-react"
...
export function ScholarshipWorkflowMermaid({
  ...
  const [scholarshipGroups, setScholarshipGroups] = useState<Record<string, ScholarshipConfiguration[]>>({})
  ...
  useEffect(() => {
    if (activeTab && scholarshipGroups[activeTab] && scholarshipGroups[activeTab].length > 0) {
      const groupConfigs = scholarshipGroups[activeTab]
      // 如果當前選中的配置不在這個組中，切換到該組的第一個配置
      if (!selectedConfig || !groupConfigs.find(c => c.id === selectedConfig.id)) {
  ...
  const loadWorkflowStats = async (configId: number) => {
    ...
    try {
      ...
      const response = await apiClient.admin.getApplicationsByScholarship(config.scholarship_type_code)
      ...
      if (response.success && response.data) {
        ...
        // 判斷目前處於哪個階段
        if (config.renewal_application_start_date && config.renewal_application_end_date) {
  ...
  const getWorkflowStages = (config: ScholarshipConfiguration) => {
    ...
    if (config.renewal_application_start_date) {
      stages.push({
        ...
        status: getStageStatus(config.renewal_application_start_date, config.renewal_application_end_date),
    ...
    if (config.application_start_date) {
      stages.push({
        name: '一般申請期',
        type: 'application',
        status: getStageStatus(config.application_start_date, config.application_end_date),
        period: `${formatDate(config.application_start_date)} - ${formatDate(config.application_end_date)}`
      })
    }

    // 教授審查階段
    if (config.requires_professor_recommendation && config.professor_review_start) {
      stages.push({
        name: '教授審查',
        type: 'professor_review',
        status: getStageStatus(config.professor_review_start, config.professor_review_end),
    ...
    if (config.requires_college_review && config.college_review_start) {
      stages.push({
        name: '學院審查',
        type: 'college_review',
        status: getStageStatus(config.college_review_start, config.college_review_end),
    ...
    stages.push({
      ...
      type: 'final_review',
      status: 'pending',
      period: config.review_deadline ? `截止: ${formatDate(config.review_deadline)}` : ''
    })
  ...
  const viewStageDetails = async (stageName: string) => {
    ...
    try {
      const response = await apiClient.admin.getApplicationsByScholarship(selectedConfig.scholarship_type_code)
      if (response.success && response.data) {
        // 根據階段篩選申請
        ...
        if (stageName.includes("申請")) {
          filteredApplications = response.data.filter(app =>
            app.status === 'submitted' || app.status === 'under_review'
          )
        } else if (stageName.includes("教授")) {
          filteredApplications = response.data.filter(app =>
            app.status === 'professor_review_pending' || app.status === 'professor_reviewed'
          )
        } else if (stageName.includes("學院")) {
          filteredApplications = response.data.filter(app =>
            app.status === 'college_review_pending' || app.status === 'college_reviewed'
          )
        }
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="flex w-full">
          {Object.keys(scholarshipGroups).map((typeName) => (
            <TabsTrigger key={typeName} value={typeName} className="flex-1 flex items-center justify-center gap-2">
              <span>{typeName}</span>
        ...
        {Object.keys(scholarshipGroups).map((typeName) => (
          <TabsContent key={typeName} value={typeName} className="space-y-4 mt-6">
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
              <h3 className="text-lg font-semibold text-blue-900 mb-2">{typeName}</h3>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center gap-4 mb-4">
                  <label className="text-sm font-medium text-gray-700">選擇配置：</label>
                  <Select
                    value={selectedConfig?.id.toString() || ""}
                    onValueChange={(value) => {
                      const configId = parseInt(value)
                      const config = scholarshipGroups[typeName].find(c => c.id === configId)
                      if (config) {
                        setSelectedConfig(config)
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-blue-800">總申請數</span>
                  </div>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-orange-800">待審核</span>
                  </div>
              ...
              <div className="bg-green-50 p-3 rounded-lg border border-green-200">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-green-800">當前階段</span>
                </div>
                <p className="text-sm font-medium text-green-900">{stats.currentStage}</p>
              </div>
              <div className="bg-purple-50 p-3 rounded-lg border border-purple-200">
                <div className="flex items-center gap-2">
                  <Calendar className="h-4 w-4 text-purple-600" />
                  <span className="text-sm font-medium text-purple-800">下次截止</span>
                </div>
                <p className="text-sm font-medium text-purple-900">
                  {stats.nextDeadline ? format(stats.nextDeadline, "MM/dd") : "無"}
                </p>
              </div>
            </div>
          )}
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        ...
                        {selectedConfig?.semester && ` ${selectedConfig.semester}學期`}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      {loading ? (
                        ...
                        <div className="w-full overflow-x-auto">
                          <div className="bg-gray-50 p-6 rounded-lg border">
                            <div className="text-center mb-6">
                              <h3 className="text-lg font-semibold text-gray-700">{selectedConfig.config_name} 工作流程</h3>
                              <p className="text-sm text-gray-500">顯示獎學金申請與審查流程</p>
                            </div>

                <div className="space-y-4">
                  {getWorkflowStages(selectedConfig).map((stage: any, index: number, array: any[]) => (
                      ...
                      `}>
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            {stage.period && (
                              <p className="text-sm text-gray-600 mt-1">{stage.period}</p>
                            )}
                          </div>
                          <div className={`
                          ...
                          `}>
                            ...
                            {stage.status === 'active' && '進行中'}
                            {stage.status === 'pending' && '未開始'}
                            {stage.status === 'expired' && '已過期'}
                          </div>
                        </div>
                      </div>
                    ...
                    <TableRow key={app.id}>
                      ...
                      <TableCell>
                        ...
                        >
                          ...
                          {app.status === 'professor_review_pending' && '教授審核中'}
                          {app.status === 'college_review_pending' && '學院審核中'}
                          {app.status === 'approved' && '已核准'}
                          {app.status === 'rejected' && '已拒絕'}
                          {!['submitted', 'under_review', 'professor_review_pending', 'college_review_pending', 'approved', 'rejected'].includes(app.status) && app.status}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        {app.created_at ? format(new Date(app.created_at), "yyyy/MM/dd HH:mm") : 'N/A'}
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
