...
class MatrixDistributionService:
    ...
    async def execute_matrix_distribution(
        ...
        Raises:
            ValueError: If ranking or configuration not found
            Exception: For database or other unexpected errors (transaction is rolled back)
        """
        try:
            # Get ranking with row-level lock to prevent concurrent distribution execution
            # This ensures only one distribution process can run at a time for this ranking
            ranking_stmt = (
            ...
            logger.info(f"Acquired row-level lock for ranking {ranking_id}")
            ...
            if not config or not config.has_college_quota or not config.quotas:
                raise ValueError(
                    f"No matrix quota configuration found for scholarship type {ranking.scholarship_type_id}"
                )
            ...
            logger.info(f"Successfully committed distribution for ranking {ranking_id}")
            ...
            # Re-raise ValueError without rollback (data validation errors)
            logger.error(f"Validation error in distribution for ranking {ranking_id}: {str(e)}")
            raise

        except Exception as e:
            # Rollback transaction on any other error
            await self.db.rollback()
            logger.error(f"Error executing distribution for ranking {ranking_id}: {str(e)}", exc_info=True)
            raise Exception(f"Failed to execute matrix distribution: {str(e)}") from e
