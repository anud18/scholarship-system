...
class GitHubIntegrationService:
    ...
    def __init__(self, db: AsyncSession):
        ...
        self.github_repo = getattr(
            settings, "GITHUB_REPO", "jotpalch/scholarship-system"
        )
    ...
    async def _generate_issue_body(
        ...
        body_parts.append(
            f"**Execution Date:** {distribution.executed_at.strftime('%Y-%m-%d %H:%M:%S UTC')}"
        )
        ...
        body_parts.append(
            f"- **Total Applications:** {distribution.total_applications}"
        )
        ...
        if distribution.distribution_summary:
            ...
            body_parts.append(
                "| Sub-Type | Total Applications | Quota | Allocated | Rejected | Utilization |"
            )
            body_parts.append(
                "|----------|-------------------|-------|-----------|----------|-------------|"
            )
        ...
        if ranking:
            ...
            if ranking_items:
                body_parts.append(
                    "| Rank | Student | Student ID | Score | Status | Allocation |"
                )
                body_parts.append(
                    "|------|---------|------------|-------|--------|------------|"
                )
                ...
                for item in ranking_items:
                    student_name = (
                        item.application.student_data.get("cname", "N/A")
                        if item.application.student_data
                        else "N/A"
                    )
                    student_id = (
                        item.application.student_data.get("stdNo", "N/A")
                        if item.application.student_data
                        else "N/A"
                    )
        ...
        body_parts.append("---")
        body_parts.append(
            "*This issue was automatically generated by the Scholarship Management System.*"
        )
    ...
    async def create_ranking_summary_issue(
        self, ranking: CollegeRanking
    ) -> Dict[str, Any]:
    ...
    async def _generate_ranking_summary_body(self, ranking: CollegeRanking) -> str:
        ...
        body_parts.append(
            f"**Created:** {ranking.created_at.strftime('%Y-%m-%d %H:%M:%S UTC')}"
        )
        body_parts.append("")
        ...
        if ranking_items:
            ...
            for item in ranking_items:
                ...
                score = f"{item.total_score:.2f}" if item.total_score else "N/A"
                review_status = (
                    item.college_review.review_status if item.college_review else "N/A"
                )
...
def generate_distribution_csv_content(ranking_items: List[CollegeRankingItem]) -> str:
    ...
    csv_lines = []
    csv_lines.append(
        "Rank,Student Name,Student ID,Score,Allocated,Status,Allocation Reason"
    )
    ...
    for item in sorted(ranking_items, key=lambda x: x.rank_position):
        ...
        csv_lines.append(
            f"{item.rank_position},{student_name},{student_id},{score},{allocated},{status},{reason}"
        )
