...
class EmailAutomationRule:
    ...
    def __init__(self, id: int, template_key: str, trigger_event: str, 
                 delay_hours: int = 0, condition_query: str = None, is_active: bool = True):
        self.id = id
...
class EmailAutomationService:
    ...
    async def get_automation_rules(self, db: AsyncSession, trigger_event: str) -> List[EmailAutomationRule]:
        ...
        for row in result:
            rules.append(EmailAutomationRule(
                id=row.id,
            ...
            try:
                ...
                if rule.delay_hours > 0:
                    ...
                    await self._schedule_automated_email(
                        db, rule.template_key, recipient['email'], recipient_context, 
                        scheduled_for, email_category, context
                    )
                else:
                    # Send immediately
                    await self._send_automated_email(
                        db, rule.template_key, recipient['email'], recipient_context,
                        email_category, context
                    )
                    
            except Exception as e:
                logger.error(f"Failed to send email to {recipient.get('email', 'unknown')}: {e}")
    
    async def _get_recipients(self, db: AsyncSession, rule: EmailAutomationRule, context: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def _send_automated_email(self, db: AsyncSession, template_key: str, recipient_email: str, 
                                  context: Dict[str, Any], email_category: EmailCategory, 
    ...
    async def _schedule_automated_email(self, db: AsyncSession, template_key: str, recipient_email: str,
                                      context: Dict[str, Any], scheduled_for: datetime,
                                      email_category: EmailCategory, trigger_context: Dict[str, Any]):
    ...
    async def trigger_application_submitted(self, db: AsyncSession, application_id: int, 
    ...
    async def trigger_professor_review_submitted(self, db: AsyncSession, application_id: int,
    ...
    async def trigger_final_result_decided(self, db: AsyncSession, application_id: int,
        ...
        try:
            ...
            for email_row in scheduled_emails:
                try:
                    ...
                    # Send the email
                    metadata = {
                        'email_category': EmailCategory(email_row.email_category) if email_row.email_category else EmailCategory.SYSTEM,
