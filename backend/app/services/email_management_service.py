...
class EmailManagementService:
    ...
    async def get_due_scheduled_emails(
        ...
        query = select(ScheduledEmail).options(
            ...
            asc(ScheduledEmail.priority),
            asc(ScheduledEmail.scheduled_for)
        ).limit(limit)
    ...
    async def process_due_emails(
        ...
        for scheduled_email in due_emails:
            ...
            try:
                ...
                logger.info(f"Successfully sent scheduled email {scheduled_email.id} to {scheduled_email.recipient_email}")
