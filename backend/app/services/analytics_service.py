...
class ScholarshipAnalyticsService:
    ...
    def _calculate_overview_metrics(
        ...
        return {
            ...
            "pending_rate": (pending / total_applications * 100)
            if total_applications > 0
            else 0,
    ...
    def _analyze_application_status(
        self, applications: List[Application]
    ) -> Dict[str, Any]:
    ...
    def _analyze_scholarship_types(
        self, applications: List[Application]
    ) -> Dict[str, Any]:
        ...
        for main_type in ScholarshipMainType:
            main_apps = [
                app
                for app in applications
                if app.main_scholarship_type == main_type.value
            ]
            if main_apps:
                ...
                for sub_type in ScholarshipSubType:
                    sub_apps = [
                        app
                        for app in main_apps
                        if app.sub_scholarship_type == sub_type.value
                    ]
                    if sub_apps:
                        main_type_stats[main_type.value]["sub_types"][
                            sub_type.value
                        ] = {
                            "total": len(sub_apps),
                            "approved": len(
    ...
    def _analyze_renewal_patterns(
        ...
        renewal_analysis = {
            ...
            "renewal_percentage": (len(renewal_apps) / len(applications) * 100)
            if applications
            else 0,
        }
        ...
        if renewal_apps:
            ...
            renewal_analysis["renewal_approval_rate"] = (
                renewal_approved / len(renewal_apps) * 100
            )
        ...
        if new_apps:
            ...
            renewal_analysis["new_application_approval_rate"] = (
                new_approved / len(new_apps) * 100
            )

        # Priority score comparison
        renewal_priority_scores = [
            app.priority_score for app in renewal_apps if app.priority_score
        ]
        new_priority_scores = [
            app.priority_score for app in new_apps if app.priority_score
        ]

        if renewal_priority_scores:
            renewal_analysis["average_renewal_priority"] = sum(
                renewal_priority_scores
            ) / len(renewal_priority_scores)

        if new_priority_scores:
            renewal_analysis["average_new_priority"] = sum(new_priority_scores) / len(
                new_priority_scores
            )
    ...
    def _calculate_performance_metrics(
        ...
        priority_scores = [
            app.priority_score for app in applications if app.priority_score is not None
        ]
        priority_distribution = {
            "high_priority": len([score for score in priority_scores if score >= 100]),
            "medium_priority": len(
                [score for score in priority_scores if 50 <= score < 100]
            ),
        ...
        for status in ApplicationStatus:
            status_apps = [
                app
                for app in applications
                if app.status == status.value and app.priority_score is not None
            ]
            if status_apps:
                avg_priority = sum(app.priority_score for app in status_apps) / len(
                    status_apps
                )
        ...
        return {
            ...
            "average_overall_priority": round(
                sum(priority_scores) / len(priority_scores), 1
            )
            if priority_scores
            else 0,
        }
    ...
    def _analyze_success_factors(
        ...
        approved_priorities = [
            ...
            if app.priority_score is not None
        ]
        rejected_priorities = [
            app.priority_score
        ...
        if approved_priorities and rejected_priorities:
            success_factors["priority_correlation"] = {
                "avg_approved_priority": round(
                    sum(approved_priorities) / len(approved_priorities), 1
                ),
                "avg_rejected_priority": round(
                    sum(rejected_priorities) / len(rejected_priorities), 1
                ),
        ...
        try:
            ...
            executive_summary = {
                ...
                "trends": {
                    ...
                    "highest_success_rate": type_analysis.get(
                        "highest_approval_rate_type"
                    ),
                    "renewal_success_advantage": renewal_analysis.get(
                        "renewal_approval_rate", 0
                    )
            ...
            if overview["approval_rate"] > 70:
                executive_summary["insights"].append(
                    "High approval rate indicates effective screening process"
                )
            ...
            if overview["average_processing_time_days"] > 30:
                executive_summary["insights"].append(
                    "Processing time exceeds target of 30 days"
                )
    ...
    async def get_predictive_insights(self, forecast_months: int = 6) -> Dict[str, Any]:
        ...
        try:
            ...
            predictions = {
                ...
                "predicted_applications": round(
                    avg_monthly_applications * forecast_months
                ),
