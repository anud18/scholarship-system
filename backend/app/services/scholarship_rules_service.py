...
class ScholarshipRulesService:
    ...
    async def validate_rule_condition(
        ...
        if operator in ['in', 'not_in']:
            try:
                ...
                if not values or any(not v for v in values):
                    return False, f"For '{operator}' operator, expected_value must be comma-separated non-empty values"
            except Exception:
        ...
        if test_data:
            try:
                ...
                return True, f"Test passed: field_value='{field_value}' {operator} '{expected_value}' = {result}"
    ...
    async def _update_rule_from_source(
        ...
        fields_to_copy = [
            'condition_field', 'operator', 'expected_value', 'message', 'message_en',
            'is_hard_rule', 'is_warning', 'priority', 'description', 'tag',
            'is_initial_enabled', 'is_renewal_enabled'
        ]
