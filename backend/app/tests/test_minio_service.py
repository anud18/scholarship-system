...
class TestMinIOService:
    ...
    @pytest.mark.asyncio
    async def test_upload_file_bucket_not_exists_creates_bucket(
    ...
    async def test_list_files_success(self, minio_service_instance, mock_minio_client):
        ...
        mock_objects = [
            Mock(
                object_name="documents/file1.pdf",
                size=1024,
                last_modified=datetime.now(),
            ),
            Mock(
                object_name="documents/file2.pdf",
                size=2048,
                last_modified=datetime.now(),
            ),
            Mock(
                object_name="documents/subfolder/file3.pdf",
                size=512,
                last_modified=datetime.now(),
            ),
        ]
        ...
        files = [
            {
                ...
                "content": BytesIO(b"content1"),
                "content_type": "application/pdf",
            },
            {
                "object_name": "file2.pdf",
                "content": BytesIO(b"content2"),
                "content_type": "application/pdf",
            },
        ]
        bucket_name = "test-bucket"
    ...
    async def test_cleanup_old_files(self, minio_service_instance, mock_minio_client):
        ...
        old_files = [
            Mock(
                object_name="temp/old_file1.pdf",
                last_modified=cutoff_date - timedelta(days=1),
            ),
            Mock(
                object_name="temp/old_file2.pdf",
                last_modified=cutoff_date - timedelta(days=2),
            ),
        ]
        recent_files = [
            Mock(
                object_name="temp/recent_file.pdf",
                last_modified=cutoff_date + timedelta(days=1),
            )
        ]
        ...
        cleaned_count = await minio_service_instance.cleanup_old_files(
            bucket_name=bucket_name, prefix=prefix, cutoff_date=cutoff_date
        )
