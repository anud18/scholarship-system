...
class TestRenewalFlowSequence:
    ...
    def test_renewal_flow_completes_before_general_starts(
        self, scholarship_with_sequential_flow
    ):
        """Test that renewal flow completes before general application starts"""
        # 驗證續領學院審查結束時間早於一般申請開始時間
        assert (
            scholarship_with_sequential_flow.renewal_college_review_end
            < scholarship_with_sequential_flow.application_start_date
        )

        # 驗證續領教授審查結束時間早於續領學院審查開始時間
        assert (
            scholarship_with_sequential_flow.renewal_professor_review_end
            < scholarship_with_sequential_flow.renewal_college_review_start
        )

        # 驗證續領申請結束時間早於續領教授審查開始時間
        assert (
            scholarship_with_sequential_flow.renewal_application_end_date
            < scholarship_with_sequential_flow.renewal_professor_review_start
        )

    def test_no_overlap_between_renewal_and_general_periods(
        self, scholarship_with_sequential_flow
    ):
    ...
    def test_sequential_review_stages(self, scholarship_with_sequential_flow):
        ...
        scholarship_with_sequential_flow.renewal_application_start_date = (
            test_time - timedelta(days=5)
        )
        scholarship_with_sequential_flow.renewal_application_end_date = (
            test_time + timedelta(days=5)
        )
        scholarship_with_sequential_flow.application_start_date = test_time + timedelta(
            days=20
        )
        ...
        scholarship_with_sequential_flow.application_end_date = test_time + timedelta(
            days=5
        )
        scholarship_with_sequential_flow.professor_review_start = test_time + timedelta(
            days=10
        )
    ...
    def test_application_eligibility_sequence(self, scholarship_with_sequential_flow):
        ...
        scholarship_with_sequential_flow.renewal_application_end_date = now + timedelta(
            days=5
        )
        scholarship_with_sequential_flow.application_start_date = now + timedelta(
            days=20
        )

        # 檢查續領申請資格
        can_renewal = scholarship_with_sequential_flow.can_student_apply_renewal(
            1, existing_applications
        )
        assert can_renewal is True

        # 檢查一般申請資格（應該被拒絕，因為還不是一般申請期間）
        can_general = scholarship_with_sequential_flow.can_student_apply_general(
            1, existing_applications
        )
