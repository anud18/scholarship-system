...
The calculate_total_received_months DB call is mocked. We're pinning the
...
def _config(code: str = "phd_scholarship", config_id: int = 1):
    ...
    return SimpleNamespace(
        id=config_id,
        # The imported 國科會 baseline is keyed by scholarship_type, so the cap
        # check reads this alongside the config id.
...
class TestCheckPhdEligibilityMonthsCap:
    ...
    @patch("app.services.plugins.phd_eligibility_plugin.calculate_total_received_months")
...
class TestCalculateReceivedMonthsFailOpen:
    ...
    def test_exception_returns_zero_via_eligibility_path(self, mock_calc):
        # Pin: when calculate_total_received_months raises, eligibility
