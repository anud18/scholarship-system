...
sync_engine = create_engine(
    ...
    } if "postgresql" in settings.database_url_sync else {},
)
...
async def invalidate_connection_pools():
    ...
    try:
        ...
        logger.info(f"Before invalidation - Async pool: {async_pool.size()} connections, Sync pool: {sync_pool.size()} connections")
...
def invalidate_connection_pools_sync():
    ...
    try:
        ...
        logger.warning("Async engine disposal requires async context - consider using invalidate_connection_pools() in async context")
