...
def upgrade() -> None:
    # Check if the table already exists (created by initial migration via Base.metadata.create_all)
    bind = op.get_bind()
    inspector = sa.inspect(bind)
    existing_tables = inspector.get_table_names()

    if 'professor_student_relationships' not in existing_tables:
        # Create professor_student_relationships table only if it doesn't exist
        op.create_table(
