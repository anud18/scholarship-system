        ...
        try:
            ...
            if form_documents:
                for i, doc in enumerate(form_documents, 1):
                    ...
                    # 檢查前端需要的所有欄位
                    required_fields = ['document_type', 'file_id', 'filename', 'is_verified']
            ...
            # 檢查固定文件的特徵
            bank_doc = next((doc for doc in form_documents if doc.get('document_type') == 'bank_account_proof'), None)
            if bank_doc:
            ...
            if bank_doc:
                ...
                if all(v is not None for v in [frontend_file['id'], frontend_file['file_type'], frontend_file['filename']]):
                    print("  ✅ 前端可以正確顯示此文件")
                    return True
                else:
                    print("  ❌ 前端無法正確顯示此文件")
