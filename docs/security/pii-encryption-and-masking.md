...
系統只從這兩個環境變數取得金鑰,不從程式碼、也不從其他檔案取得。
程式碼裡沒有任何一把正式環境的金鑰。

```
PII_ENCRYPTION_KEYS={"v1":"<金鑰>","v2":"<金鑰>"}
PII_ENCRYPTION_ACTIVE_VERSION=v2
```

**`PII_ENCRYPTION_KEYS`——所有可用的金鑰**

| 項目 | 規定 |
|---|---|
| 格式 | 一個 JSON 物件,寫成「版本名稱: 金鑰」的對應表 |
| 版本名稱 | 自訂字串,慣例用 `v1`、`v2`、`v3` 依序往下 |
| 金鑰本體 | 32 位元組(256 位元)隨機值,以 base64url 編碼後填入 |
| 要放幾把 | 目前在用的那一把,加上所有「還有資料是用它加密的」舊金鑰 |

系統啟動時會逐一檢查:不是合法 JSON、版本名稱空白、金鑰不是合法 base64url、
或解碼後長度不是 32 位元組,都會直接讓服務啟動失敗,不會帶著壞掉的金鑰上線。

**`PII_ENCRYPTION_ACTIVE_VERSION`——新資料要用哪一把**

- 填版本名稱,例如 `v2`。之後新寫入的身分證字號一律用這一把加密。
- 這個版本必須存在於 `PII_ENCRYPTION_KEYS` 裡,否則系統報錯。
- 沒設定時預設為 `v1`。
- 舊資料不受影響:每一筆加密字串裡都標了自己的版本,系統照那個版本去找金鑰解密。
  所以**舊金鑰只要還留在 `PII_ENCRYPTION_KEYS` 裡,舊資料就解得開**。

兩個變數都是在服務啟動時讀取並存在記憶體中,**改完必須重新部署或重啟服務才會生效**。
...
1. **誰拿得到金鑰。** 能存取正式版本庫環境設定的人、能核准或修改部署流程的人,
   ...
   設定檔時,範圍縮小為能登入該主機的人。
2. **金鑰遺失只影響申請資料,不影響稽核軌跡。** 稽核紀錄在寫入時就把身分證
   字號換成 `[REDACTED]`,不含密文。
3. **年度稽核若要調閱三年以上的舊申請資料**,前提是第四節規則 2 有被遵守
   (舊金鑰還留著)。
