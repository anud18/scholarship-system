# NYCU Scholarship System — 正式機更新手冊 (v1.0.7 )
...
### 3.5 AP VM 以及 DB VM 都要做：把使用者加進 docker group（重點修正）
...
SUPER_ADMIN_NYCU_ID=CHANGE_ME_NYCU_ID
...
```bash
sudo ./svc.sh install nycuaa
sudo ./svc.sh start
# 裝完後確認服務狀態，並確認能不加 sudo 操作 docker：
sudo systemctl status actions.runner.*
sudo -u nycuaa -i docker ps   # 應該要成功，不用密碼
```
...
### 附錄：移除 Runner
有 sudo 的帳號
先進到原先安裝 runner 的目錄下執行
```bash
sudo ./svc.sh uninstall
```

進到 `Setting` -> `Actions` -> `Runners` 對要移除的 runner 點三個圓點點, 按下去會有 Remove runner 點下去
<img width="1916" height="521" alt="image" src="https://github.com/user-attachments/assets/87216b56-5e53-46b2-88f1-3790e3a1bd20" />
按下去會有一行指令請執行
<img width="786" height="493" alt="image" src="https://github.com/user-attachments/assets/df6f3bb4-adf6-4f4d-ba04-1b8620218cdc" />
...
## 附錄：清空並重建 DB volume（銷毀性）

**這一節會刪掉資料庫裡的全部資料。** 動手前先讀完「先確認你真的需要」。

### 先確認你真的需要

多數想重建 volume 的情境，其實有不銷毀資料的解法：

| 症狀 | 先試這個 | 真的要重建嗎 |
| --- | --- | --- |
| `password authentication failed` | `ALTER USER scholarship_user PASSWORD '...'` | **不用** |
| 改了 `.env` 的 `POSTGRES_PASSWORD` 卻沒生效 | 同上 | **不用** |
| 想換 `POSTGRES_DB` / `POSTGRES_USER` | `CREATE DATABASE` / `CREATE ROLE` | **不用** |
| 資料目錄權限錯亂 | `chown -R 70:70 /opt/scholarship/postgres` | **不用** |
| 資料損毀、initdb 失敗、要從乾淨狀態重來 | — | 要 |

> **`POSTGRES_PASSWORD` 只在 volume 第一次初始化時生效。** 資料目錄一旦有內容，postgres 的 entrypoint 就會跳過 `initdb`，之後不管把 `.env` 改成什麼、`up -d` 幾次，資料庫裡存的密碼都不會變。這是「密碼改了沒用」最常見的原因，而它的正解是 `ALTER USER`，不是重建。

改密碼的非銷毀作法：

```bash
docker exec scholarship_postgres_prod psql -U scholarship_user -d scholarship_db \
  -c "ALTER USER scholarship_user PASSWORD '<新密碼>'"
```

改完記得把 `.env` 的 `POSTGRES_PASSWORD` 和 `DB_PASSWORD`、以及 AP VM `~/.env` 的 `DB_PASSWORD` 一起改成同一個值，否則下次重建 volume 又會不一致。

### ⚠️ 陷阱：`docker volume rm` 不會刪掉資料

`docker-compose.prod-db.yml` 裡的三個 volume 都是 **bind 型**：
...
真正的資料在主機的 `/opt/scholarship/postgres/data`，volume 只是一個指標。所以：

- `docker compose down -v`、`docker volume rm postgres_prod_data` — **只刪掉指標，資料一個位元組都沒少**
- 下次 `up -d` 會重建同名 volume、指回同一個目錄，postgres 看到 PGDATA 非空 → **跳過 initdb** → 密碼、資料全部照舊

也就是說，照直覺做完整套「移除 volume 再重建」，結果會是**什麼都沒發生**，而且沒有任何錯誤訊息。要真的重來，必須清空主機目錄。
...
docker exec scholarship_postgres_prod pg_dump -U scholarship_user -d scholarship_db \
  --no-owner --no-acl | gzip > ~/pre-rebuild-$(date +%Y%m%d-%H%M%S).sql.gz

ls -lh ~/pre-rebuild-*.sql.gz
gunzip -t ~/pre-rebuild-*.sql.gz && echo "壓縮檔完整"
```

`pg_dump` 走容器內的 unix socket（`local` 是 trust），不需要密碼——**即使現在 TCP 連線因為密碼錯誤而失敗，這一步一樣做得到**。

> 檔案大小如果只有幾 KB，代表資料庫其實是空的，先弄清楚原因再往下走。

#### 1. 停掉會連 DB 的服務

```bash
docker compose --env-file .env -f docker-compose.prod-db-monitoring.yml down
docker compose --env-file .env -f docker-compose.prod-db.yml down
```

AP VM 那邊的 backend 也應該一併停掉，避免它在重建過程中不斷重試連線。

#### 2. 清空資料目錄 ← 真正生效的一步

```bash
# 先看一眼，確認是對的目錄（應該看得到 base、global、pg_wal 等）
sudo ls -A /opt/scholarship/postgres/data | head

sudo rm -rf /opt/scholarship/postgres/data
sudo mkdir -p /opt/scholarship/postgres/data
sudo chown -R 70:70 /opt/scholarship/postgres
```

用「整個刪掉再建回來」而不是 `rm -rf .../data/*`，是因為後者不會刪到隱藏檔，留下的殘檔一樣會讓 postgres 跳過 initdb。

> **只清 `data`，絕對不要碰 `/opt/scholarship/postgres/backups`** ——那裡放的是既有備份，是這次操作出錯時的唯一退路。
>
> **RustFS 的 `/opt/scholarship/rustfs/data` 也不要動**，上傳的檔案在那裡，跟資料庫是分開的。

#### 3. 確認 `.env` 的密碼就是你要的

initdb 會用當下 `.env` 的 `POSTGRES_PASSWORD` 建立 superuser，所以這是唯一能決定新密碼的時機。

```bash
# 三份雜湊前綴應該相同（只印雜湊，不會外洩密碼本身）
sed -n 's/^POSTGRES_PASSWORD=//p' .env | tail -1 | tr -d '\r\n' | sha256sum | cut -c1-12
sed -n 's/^DB_PASSWORD=//p'       .env | tail -1 | tr -d '\r\n' | sha256sum | cut -c1-12
# AP VM 上：
# sed -n 's/^DB_PASSWORD=//p' /home/nycuaa/.env | tail -1 | tr -d '\r\n' | sha256sum | cut -c1-12
```

不一致就先改成一致再往下，否則重建完 backend 依然連不進來。

#### 4. 重新啟動，並確認 initdb 真的跑了

```bash
docker compose --env-file .env -f docker-compose.prod-db.yml up -d
docker logs -f scholarship_postgres_prod
```
...
> **如果沒有看到 `initdb`，直接跳到 `database system is ready`，代表目錄沒清乾淨，這次重建無效。** 回到步驟 2 重做。

#### 5. 驗證新密碼

```bash
read -rs -p "密碼: " PW; echo
docker run --rm --network scholarship_prod_db_network -e PGPASSWORD="$PW" \
  postgres:15-alpine psql -h postgres -U scholarship_user -d scholarship_db -c 'select 1'
unset PW
```
...
```bash
gunzip -c ~/pre-rebuild-*.sql.gz | \
  docker exec -i scholarship_postgres_prod psql -U scholarship_user -d scholarship_db
```

`pg_dump --no-owner --no-acl` 產出的 dump **不含 role 密碼**，所以還原不會把舊密碼帶回來，步驟 3 設定的新密碼會保留。

**選項 B：從空資料庫開始**（不要舊資料）

schema 由 AP VM 的部署流程建立——`Deploy to Production` 裡的 `Run database migrations` 步驟會跑 alembic。直接重跑一次部署即可。
...
15 秒後 `postgres_exporter` 的 `pg_up` 應該變成 `1`，Grafana 上直接看得到。這是整套流程最乾淨的驗證訊號：
...
- [ ] `docker compose --env-file .env -f docker-compose.prod-db.yml ps` 全部 healthy
- [ ] `sudo ls /opt/scholarship/postgres/data` 看得到 `base`、`global`
- [ ] 步驟 5 的 `select 1` 通過
- [ ] `curl -s http://localhost:9187/metrics | grep '^pg_up'` 是 `pg_up 1`
- [ ] AP VM 的 backend healthy、`https://ss.aa.nycu.edu.tw` 可登入
- [ ] `~/pre-rebuild-*.sql.gz` 確認保留，等系統穩定數日後再刪
