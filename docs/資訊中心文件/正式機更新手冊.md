...
- **輸入/輸出連線**（建議僅開必要埠）：
  - VM-A（App/Proxy）：對外只開 `443/tcp`。若需要直接連線，對 VM-B 開啟 `5432/tcp`（PostgreSQL）與 `9000-9001/tcp`（RustFS API/Console）。走私有網段或 VPC Peering 時，請限制來源 IP。
  - VM-B（DB/Storage）：只允許 **VM-A 私網 IP** 存取 DB 與 RustFS，不要對公網開放。若有起監控（第 4.7 節），會再多開 `9100/tcp`（node-exporter）與 `9187/tcp`（postgres-exporter），這兩個只開給監控主機的私網 IP。
- **NTP**：兩台 VM 時間要同步，避免 JWT/OIDC 與 TLS 出問題。
...
這組會對外發布 `9100`（node-exporter）和 `9187`（postgres-exporter）兩個埠。跟 5432/9000/9001 一樣，**只開給監控主機的私網 IP，不要對公網開**。
