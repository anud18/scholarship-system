...
| #64 professor review lock after college starts | local dev stack: PUT /professor/applications/1/review/1 at college_review stage → HTTP 403; stages college_reviewed / college_ranking / admin_review / completed all → 403; professor_review → 200. UI lock banner shows ("學院審核中"). Two follow-up gaps filed as issue #92, fixed in commit 34c54a1. | ✅ pass (backend guard + UI) |
...
| #55 bank document delete (de5f14fb)         | local dev stack: stuphd001 uploaded PDF → DB photo_url+object_name set, MinIO object created; DELETE /me/bank-document → both DB fields null, MinIO log "Deleted file ...", re-fetch → 404 | ✅ pass |
